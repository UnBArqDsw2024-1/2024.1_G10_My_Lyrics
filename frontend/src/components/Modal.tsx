...
export default function Modal({ isOpen, onClose, playlist }: ModalProps) {
  ...
  async function handleAddMusic(musicId: string) {
    try {
      const res = await api.post(`/playlist/${playlist.id}/music`, {
        musicId,
      });
      console.log(res);
    } catch (error) {
  ...
  return (
    isOpen && (
      <div className="fixed inset-0 z-50 overflow-y-auto bg-black/50 backdrop-blur-sm transition-opacity duration-300 flex items-center justify-center">
        <div className="flex min-h-screen items-center justify-center p-4 text-center w-2/4">
              ...
              <div className="mt-5 sm:mt-6">
                <form className="w-full flex flex-col gap-8">
                  ...
                  {filteredMusic.length > 0 && (
                    <ul className="bg-[#3F3352] rounded-lg max-h-40 overflow-y-auto mt-2">
                      {filteredMusic.map((music) => (
                        <li
                          key={music.id}
                          onClick={() => handleAddMusic(music.id)}
                          className="cursor-pointer hover:bg-[#564a66] px-4 py-2"
                        >
                          {music.title}
                        </li>
                      ))}
